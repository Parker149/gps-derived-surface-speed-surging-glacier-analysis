...
% -- Point to directory --
addpath('/Volumes/volume/Data/The_GPS_Archive/SK/Filtered_Velocities_FINAL/');
...
% -- Directories -- 
addpath('/Volumes/volume/Data/weather/SK/Non_Model_Climate_data/');
addpath('/Volumes/volume/Data/weather/SK/model_data_used_for_thesis/precip/');
...
% -- Rock10 data -- 
addpath('/Volumes/volume/Data/The_GPS_Archive/SK/Rock_data/GRock_2108/');
...
% -- Directory -- 
addpath('/Volumes/volume/Data/The_GPS_Archive/SK/Unfiltered_Velocities/');
...
addpath('/Volumes/volume/Data/weather/SK/model_data_used_for_thesis/temp/');
temp = readtable('temp11_2208.csv');
addpath('/Volumes/volume/Data/weather/SK/model_data_used_for_thesis/MWE/');
...
for i = 1:length(V_names)
    siteName_V = V_names{i};
    working_V = detrend_data.(siteName_V);
    speed_up_data = data_peaks.(siteName_V);
    length_of_record = days(working_V.x(end) - working_V.x(1));
    mean_speed = nanmean(working_V.y);
    anomaly_count = length(speed_up_data.SpeedPeakY);
    anomalies_per_week = anomaly_count / (length_of_record/7);
    everage_speed_peak = nanmean(speed_up_data.SpeedPeakY);
    GPS_Statistics.(siteName_V).length_of_record = length_of_record;
    GPS_Statistics.(siteName_V).mean_speed = mean_speed;
    GPS_Statistics.(siteName_V).anomaly_count = anomaly_count;
    GPS_Statistics.(siteName_V).anomalies_per_week = anomalies_per_week;
    GPS_Statistics.(siteName_V).everage_speed_peak = everage_speed_peak;
    clear length_of_record mean_speed anomaly_count anomalies_per_week everage_speed_peak
end

%% --- Save Above Data as Matlab Struct ---
...
save(fullfile(saveDir, 'GPS_Stats.mat'), 'GPS_Statistics'); % <------ input here [file name]
