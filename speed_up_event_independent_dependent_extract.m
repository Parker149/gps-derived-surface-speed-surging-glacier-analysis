...
% -- Load Temp Data --
dataDir = '/Volumes/volume/Data/weather/SK/model_data_used_for_thesis/temp/';
site_T = loadTemp(dataDir);
T_names = fieldnames(site_T);

% -- Load MWE Data --
dataDir = '/Volumes/volume/Data/weather/SK/model_data_used_for_thesis/MWE/';
...
for ii = 1:length(V_names)
    ...
    for CM = 1:length(working_peaks.SpeedPeakX)
        ...
        working_peaks.Precip_volume(CM) = event_precip_volume(:)*1000;
        working_peaks.pre_V_trend(CM) = trendy;
        working_peaks.lag_time_precip_Rt(CM) = hours(lag_1(:));
        ...
        working_peaks.MaxMWE(CM) = (idty_MWE(:)/3)*1000;
